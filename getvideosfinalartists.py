...
def updateViewCountForVideos(item):
    #retitem = {}
    try:
        ...
        item['statistics'] = videoEntry['statistics']
        item['contentDetails'] = videoEntry['contentDetails']
        item['status'] = videoEntry['status']
        #retitem['items'] = item
    except Exception, e:
        logger_error.exception(e)
    return item
...
def runYoutubeApi(directory):
    try:
        ...
        if(return_search_results != None):
            print len(return_search_results['items'])
            #print parallel_songs_list
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                return_search = executor.map(updateViewCountForVideos,return_search_results['items'])
                return_search_results['items'] = list(return_search)
...
def getYoutubeUrl(video,flag,mostpopular,return_search_results):
    ...
    try:
        ...
        if(len(return_search_results) >250):
            bSearchAgain = True
        if(bSearchAgain):
            if(flag == 0):
            ...
            searchResult = return_search_results
        now = datetime.now()
        try:
