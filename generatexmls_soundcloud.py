...
logger_urls = loggingmodule.initialize_logger('urls','souncloudurls.csv')
    ...
    try:
        ...
        if vid.has_key('published'):
            ...
            if(m != None):
                ...
                ydate = '1001-01-01 00:00:00'
        else:
			ydate = '1001-01-01 00:00:00'
        print ydate
        crdt = datetime.strptime(ydate,'%Y-%m-%d %H:%M:%S')
        mysong.set_Id(str(vid['id']))
        mysong.set_artistId(int(curr_artist_id))
        mysong.set_songName(songName)	
        mysong.set_SCName(vid['title'])
        mysong.set_videoDate(crdt)
        ...
        soundcloudDetails = vid['sclist']
        print(len(soundcloudDetails))
        for aud in soundcloudDetails:
            #print aud
            audioDetails = api.soundcloudAudio()
            audioDetails.set_playback_count(aud['ViewCount'])
            audioDetails.set_url(aud['url'])
            audioDetails.set_scName(aud['title'])
            audioDetails.set_Id(aud['id'])
            audioDetails.set_totalMatch(aud['tm'])
            audioDetails.set_songMatch(aud['sm'])
            audioDetails.set_artistMatch(aud['am'])
            audioDetails.set_overLap(aud['match'])
            audioDetails.set_tags(aud['tags'])
            fw.write(str(aud['id']))
            fw.write('\n')
            #audioDetails.set_url(aud['url'])
            audioList.add_soundcloudAudio(audioDetails)
        fw.close()
        #print(len(audioList))
        #audioList.export(sys.stdout,0)
        ...
        '''if('length' in vid):
            mysong.set_duration(timedelta(milliseconds=int(vid['length'])))
            print mysong.get_duration()'''
        if vid.has_key('viewcountRate'):
            mysong.set_viewcountRate(vid['viewcountRate'])
        path = opdir
        if not os.path.exists(path):
			os.mkdir(path)
        fname = os.path.join(path,str(vid['id']) + ".xml")
